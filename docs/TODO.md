...
- [完了] ゲスト利用者向けログイン導線の文言を調整する
  - メイン画面のログイン導線を「ログイン」にし、E2E を更新する。
- [完了] ログアウト後の遷移先をホームへ変更する
  - ログアウト完了後にログイン画面ではなくホームへ戻る。
- [完了] CI の npm ci が通るよう lockfile を同期する
  - package.json と package-lock.json の依存解決結果を一致させる。
- [完了] ゲスト利用E2Eのタスク表示確認を安定化する
  - 同名テキストが複数表示されても、対象タスクの listitem にスコープして確認する。
...
- [完了] 本番 CD を GitHub-hosted build / Self-hosted deploy に分割する
  - 品質検査・image build・GHCR push は GitHub-hosted Runner で実行する。
  - OCI Self-hosted Runner は image pull・Compose 更新・失敗時診断だけを実行する。
